...
function Write-CocoRemoteShim([string]$FileName) {
  $remote = "$BaseUrl/onlyoffice/$($FileName)?v=ms8wofto"
  ...
  $html = $html -replace '(src=["''].*?/onlyoffice/assets/.*?\.js)(["''])', '$1?v=ms8wofto$2'
  $html = $html -replace '(href=["''].*?/onlyoffice/assets/.*?\.css)(["''])', '$1?v=ms8wofto$2'
  if ($ApiBaseUrl) {
...
Write-Host "  $BaseUrl/onlyoffice/?v=ms8wofto"
if ($ApiBaseUrl) {
  Write-Host "API:"
