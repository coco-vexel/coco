...
function Write-CocoRemoteShim([string]$FileName) {
  $remote = "$BaseUrl/onlyoffice/$($FileName)?v=ms7grzot"
  ...
  $html = $html -replace '(src=["''].*?/onlyoffice/assets/.*?\.js)(["''])', '$1?v=ms7grzot$2'
  $html = $html -replace '(href=["''].*?/onlyoffice/assets/.*?\.css)(["''])', '$1?v=ms7grzot$2'
  if ($ApiBaseUrl) {
...
Write-Host "  $BaseUrl/onlyoffice/?v=ms7grzot"
if ($ApiBaseUrl) {
  Write-Host "API:"
