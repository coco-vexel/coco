...
function Write-CocoRemoteShim([string]$FileName) {
  $remote = "$BaseUrl/onlyoffice/$($FileName)?v=ms8si26h"
  ...
  $html = $html -replace '(src=["''].*?/onlyoffice/assets/.*?\.js)(["''])', '$1?v=ms8si26h$2'
  $html = $html -replace '(href=["''].*?/onlyoffice/assets/.*?\.css)(["''])', '$1?v=ms8si26h$2'
  if ($ApiBaseUrl) {
...
Write-Host "  $BaseUrl/onlyoffice/?v=ms8si26h"
if ($ApiBaseUrl) {
  Write-Host "API:"
