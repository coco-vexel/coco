...
function Write-CocoRemoteShim([string]$FileName) {
  $remote = "$BaseUrl/onlyoffice/$($FileName)?v=ms7gwg7x"
  ...
  $html = $html -replace '(src=["''].*?/onlyoffice/assets/.*?\.js)(["''])', '$1?v=ms7gwg7x$2'
  $html = $html -replace '(href=["''].*?/onlyoffice/assets/.*?\.css)(["''])', '$1?v=ms7gwg7x$2'
  if ($ApiBaseUrl) {
...
Write-Host "  $BaseUrl/onlyoffice/?v=ms7gwg7x"
if ($ApiBaseUrl) {
  Write-Host "API:"
