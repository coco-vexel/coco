...
write_remote_loader() {
  file_name="$1"
  html="$(curl -fsSL "$BASE_URL/onlyoffice/$file_name?v=ms8vbr9y" \
    | sed -e 's#\(["'\''"]\)\./v1/#\1../v1/#g' \
          -e "s#\([\"']\)\./assets/#\1$BASE_URL/onlyoffice/assets/#g" \
    | sed -E "s#(src=[\"'][^\"']*/onlyoffice/assets/[^\"']+\.js)([\"'])#\1?v=ms8vbr9y\2#g" \
    | sed -E "s#(href=[\"'][^\"']*/onlyoffice/assets/[^\"']+\.css)([\"'])#\1?v=ms8vbr9y\2#g")"
  if [ -n "$API_BASE_URL" ]; then
